...
def _write_toc(book, instructions, history):
    if not book.get('toc'):
        ...
        if not book.get('toc'):
            raise ValueError("Table of contents generation failed")
...
def _write_chapter(book, chapter, history):
    ...
    if not chapter.get('content'):
        ...
        if not chapter.get('content'):
            raise ValueError(f"Chapter {chapter['number']} content generation failed")
...
def _write_section(book, chapter, section, history):
    ...
    if not section.get('content'):
        ...
        if not section.get('content'):
            raise ValueError(f"Section {section['number']} content generation failed")
